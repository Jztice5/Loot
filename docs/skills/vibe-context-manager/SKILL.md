...
| 现在进行时上下文 | 当前真实状态、焦点、阻塞和本机验证 | development/memory.md、当前需求条目、实时 Git/runtime |
| 未来规划上下文 | 尚未发生的需求、顺序和验收标准 | planning、architecture open questions |

architecture、AGENTS 和 runbooks 属于跨时间稳定上下文，从过去和现在提炼，并约束当前与
未来。任务开始时在 planning 原条目把 Planned/Deferred 转为 In Progress，并把项目级当前
焦点链接到 memory；任务完成、取消或被替代时转为 Done/Rejected/Superseded，证据进入
