...
当前季度执行队列必须与 Planned、In Progress、Blocked 需求集合一致，不得重复；同一时间
最多一个需求处于 In Progress。Deferred 和终态需求不进入执行队列。
...
- Markdown 本地链接目标存在。
- 季度目录和总览、需求管理文件命名正确。
- 当前季度执行队列与需求状态一致、没有重复，且最多一个 In Progress。
...
- 当前季度执行队列与需求状态一致，且最多一个 In Progress。
