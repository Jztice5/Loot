...
| 现在进行时上下文 | 当前正在做什么、阻塞和恢复点是什么 | development/memory.md、In Progress 需求、实时 Git 与运行结果 | In Progress、Blocked | 保持短小，旧状态立即替换或下沉 |
| 未来规划上下文 | 接下来做哪些需求、依赖、队列和验收是什么 | 需求管理、architecture open questions | Planned、Deferred | 明确尚未发生，不宣称已验证 |
...
3. 在 `development/memory.md` 记录当前执行、阻塞、恢复点和需求链接，不复制后续队列。
...
5. memory 清空已结束的活跃执行；下一项仍由需求管理中的计划队列决定。
...
5. memory 只保留当前执行、恢复点和最近有效验证，不复制未完成清单或计划队列。
...
- 当前它是未来规划上下文，状态为 Planned，并在需求管理队列中定义顺序和验收标准。
- 开始实现时改为 In Progress，并进入 memory 的当前执行区。
- 实现和验证期间，memory 只记录活跃状态与恢复点；详细推理进入当日 development log。
