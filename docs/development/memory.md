...
- 已建立核心 contracts、Signal State Machine v0.1、Crypto 行情 Provider v0.1，并完成
  REQ-0009 地基不变量修正和 REQ-0006 Crypto Golden Cases V0.1。
...
- 活跃需求：无；`REQ-0006` 已完成，当前没有 In Progress 需求。
- 恢复动作：从需求队列首项启动 `REQ-0005` Crypto Candidate PreFilter，直接参数化复用
  `tests/golden/crypto`，不得复制或改写 Golden Case 预期。
...
- `PYTHONPATH=src python -m unittest discover -s tests -p 'test_*.py' -v`：45 个测试通过。
- `python -m compileall -q src tests scripts`：通过。
- `python scripts/check_context.py`：通过；49 个 Markdown、126 个本地链接，Skill 镜像一致。
