...
- 当前代码入口：`main.py` 仍是示例；核心代码位于 `src/loot/contracts`、
  `src/loot/runtime`、`src/loot/signals` 和 `src/loot/domains/crypto`。
- 需求状态与执行队列：[需求管理 2026-Q3](../planning/2026-Q3/需求管理-2026-Q3.md)。

## 当前执行与恢复点

- 当前没有 In Progress 需求；下一项工作需要先在需求管理中登记。
- `REQ-0012` 已完成 Crypto Skill Runtime 最小基线。后续可在 Alert Center 与 Agent/LLM 编排
  之间单独登记下一需求；Crypto 初版闭环仍优先补 Alert 和最小 Replay。
...
- Skill Runtime 最小基线已实现；Agent、Alert 和最小 Replay 尚未实现。
...
- `& .\.venv\Scripts\python.exe -m pytest -q`：85 passed（包含 7 个真实 PostgreSQL 集成测试）。
...
- `& .\.venv\Scripts\python.exe scripts\check_context.py`：通过，63 个 Markdown、156 个本地链接，
  项目 Skill 与本机镜像一致。
