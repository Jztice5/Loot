...
- 当前阶段：Phase 0 架构基础阶段，已建立核心 contracts 和 Signal State Machine v0.1。
- 当前代码入口：`main.py` 仍是 PyCharm 示例脚本；核心契约代码在 `src/loot/contracts`，状态机代码在 `src/loot/signals`。
...
- `$env:PYTHONPATH='D:\my-projects\Loot\src'; py -3.12 -m unittest discover -s tests -p 'test_*.py'`：17 tests OK。
...
1. 做 FakeProvider 和 FakePreFilter，为第一条 Candidate 准备输入。
2. 用 Crypto 自选跑通一条 Candidate 到 Signal 的最小闭环。
3. 补第一批 Golden Case，验证重复事件不重复迁移和不重复提醒。
