...
当前项目处于 Phase 0 架构基础阶段，已建立 Python 项目骨架、第一批核心 contracts、Signal State Machine v0.1 和 Crypto 行情数据 Provider v0.1。现有 `main.py` 仍是 PyCharm 示例脚本，下一步会围绕 FakeCryptoPreFilter 和 Golden Case 搭第一条可运行闭环。
...
1. 做 FakeCryptoPreFilter。
2. 用 Crypto 自选跑通 Candidate 到 Signal 的最小闭环。
3. 准备 Golden Case，验证重复行情、状态迁移和提醒去重。
